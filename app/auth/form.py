...
class RegistrationForm(FlaskForm):
    ...
    password = PasswordField('密码', validators=[DataRequired(), Length(8, 24, message='密码必须是8-24位'),
...
class ChangePasswordForm(FlaskForm):
    ...
    password = PasswordField('新密码', validators=[DataRequired(message='密码不能为空'),
                                                EqualTo('password2', message='两次输入必须相同')])
...
class PasswordResetForm(FlaskForm):
    password = PasswordField('新密码', validators=[
        DataRequired(), EqualTo('password2', message='输入密码不一样')])
...
class ChangeEmailForm(FlaskForm):
    ...
    password = PasswordField('密码', validators=[DataRequired()])
    submit = SubmitField('变更邮箱地址')

    def validate_email(self, field):
        if User.query.filter_by(email=field.data).first():
            raise ValidationError('改邮箱已被使用.')
